# -*- coding: utf-8 -*-
"""MTBTGRUattention.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1-zUNSBZva7D6QgDLD0apDt3rnpP1ci-Q
"""
...
####   extracting telugu and english sentences
inputs=[]
output_s=[]
para_en = []
para_te = []
for x in range(len(captions[0:5000])):
  sentence = []
  sent_te = []
  for y in captions[x].split(' ')[0:25]:
    sentence.append(y)
  for y in telugu[x].split(' ')[0:25]:
    sent_te.append(y)
...
encoder_net = Encoder(input_size_encoder, 128,256,256,1, enc_dropout)
...
torch.save(model.state_dict(),'/content/drive/MyDrive/Project_files/mtgruattent5.pth')

def evaluation(sentence,model):
    #sentence= ['ఈ', 'కాలము', 'భారతదేశము', 'ఓ', 'పెద్ద', 'సాంఘిక', 'మార్పు', 'చవిచూసినది.']
    # sentence = []
...
model.load_state_dict(torch.load('/content/drive/MyDrive/Project_files/mtgruattent5.pth'))

sentence = input()
evaluation(sentence,model)
...
# references = []
# references = " ".join(paraphased_sentence)
# print(pred)
# print(paraphased_sentence)

import  nltk.translate.bleu_score as bleu

# reference_translation=[predictions.split()]
# candidate_translation_1=references.split()
...
print(outputs[25],inputs[25])

print(max_length)

# sentence= "एक मोटरसाइकिल दूसरे मोटर साइकिल के आगे एक पार्किंग की जगह में खड़ी हुई है"
# words_index=[]
# # words = re.findall(r'\b\w+\b', sentence)
# # print(words)
# # sentence=w
...
# sentence=[inputV_to_txt[token] for token in sentence]
# sentence.insert(0, inputV_to_txt["<s>"])
# tokens.append(inputV_to_txt["<e>"])
# sentence_tensor = torch.LongTensor(sentence).unsqueeze(1)
# with torch.no_grad():
#         hidden, cell = MODEL.encoder(sentence_tensor)
# print(inputV_to_txt["<s>"])
