...
Original file is located at
    https://colab.research.google.com/drive/1oHy-650CghQ9z37PYQnquxBkXh_qlubo
"""
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, embedding_size, hidden_size, decoder_hidden_size, num_layers, dropout):
        ...
        self.fc = nn.Linear(hidden_size * 2, decoder_hidden_size).to(device)
        self.embedding = nn.Embedding(input_size, embedding_size).to(device)
        self.rnn = nn.LSTM(embedding_size, hidden_size,bidirectional=True).to(device)
    ...
    def forward(self,x):
        ...
        temp = torch.cat((hidden[0][-2,:,:], hidden[0][-1,:,:]), dim = 1).to(device)
        temp1 = torch.cat((hidden[1][-2,:,:], hidden[1][-1,:,:]), dim = 1).to(device)
       
        # # # print(np.shape(temp))
        hidden1 = torch.tanh(self.fc(temp)).to(device)
        hidden2 = torch.tanh(self.fc(temp1)).to(device) 
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.attn = nn.Linear((encoding_hidden_size * 2) + hidden_size, hidden_size).to(device)
        self.embedding = nn.Embedding(output_size, embedding_size).to(device)
        self.v = nn.Linear(hidden_size, 1, bias = False).to(device)
        self.rnn = nn.LSTM((encoding_hidden_size * 2) + embedding_size, hidden_size).to(device)
        self.fc = nn.Linear((encoding_hidden_size * 2) + hidden_size + embedding_size , output_size).to(device)
...
#         print('hi',hidden.shape, cell.shape[0])
        hidden1 = hidden[0].unsqueeze(1).repeat(1, cell.shape[0], 1).to(device)
        # print(np.shape(hidden[0]))
        # print(np.shape(hidden1),"hidden1")
        cell1 = cell.permute(1, 0, 2).to(device)
        # print(np.shape(cell1),"cell1")
        temp = torch.cat((hidden1, cell1), dim = 2).to(device)
        # print(np.shape(temp),"temp")
        energy = torch.tanh(self.attn(temp)).to(device)
        # print(np.shape(energy),"energy")
        attention = self.v(energy).squeeze(2).to(device)
        # print(np.shape(attention),"attention")
        a = F.softmax(attention, dim=1).to(device)
        # print(np.shape(a),"a")
        a = a.unsqueeze(1).to(device)
        # print(np.shape(a),"SS")
        cell1 = cell.permute(1, 0, 2).to(device)
        # print(np.shape(cell1),"cell1")
        weighted = torch.bmm(a, cell1).to(device)
        # print(np.shape(weighted),"weighted")
        weighted = weighted.permute(1, 0, 2).to(device)
        # print(np.shape(weighted),"weighted")
        # print(np.shape(embedding),np.shape(weighted))
        rnn_input = torch.cat((embedding, weighted), dim = 2).to(device)
        # print(np.shape(rnn_input),"rr")
#         hidden = hidden.reshape(1, hidden.shape[0], hidden.shape[1])
#         print(rnn_input.shape, hidden.shape)
        hidden1=hidden[0].unsqueeze(0)
        hidden2=hidden[1].unsqueeze(0)
        hidd=[hidden1,hidden2]
        output, hidden = self.rnn(rnn_input, hidd)
        # print(np.shape(hidden[0]))
        # print(np.shape(output),np.shape(hidden))
        x = torch.cat((output.squeeze(0), weighted.squeeze(0), embedding.squeeze(0)), dim = 1).to(device)
        predictions = self.fc(x).to(device)
        hid1=hidden[0].squeeze(0)
        hid2=hidden[1].squeeze(0)
        hid=[hid1,hid2]    
        return predictions, hid
...
def train(model, iterator, optimizer, criterion, clip):
    ...
    if initial > val_loss:
      initial=val_loss
      torch.save(model.state_dict(),'/content/drive/MyDrive/Project_files/lstmattention.pth')
...
# evalutating the model for sentence
sentence=""
words_index=[]
...
model.load_state_dict(torch.load('/content/drive/MyDrive/Project_files/lstmattention.pth'))
